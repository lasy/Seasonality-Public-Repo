...
```{r child = '4a_births_models_math_description.Rmd', cache=TRUE}
```
...
```{r birth_models relative sex counts}
...
```{r birth-models-raw-relative-sex-counts, fig.width = 7, fig.height=5, fig.cap="Number of sex logs divided by the number of active users (users from all age and on any birth control type)"}
...
```{r birth_models trend in relative sex counts}
...
```{r birth-model-trend-rel-sex, fig.cap= "Relative sex counts (users from all age and on any birth control type) and trend in relative sex counts"}
...
```{r birth-model-trend-rel-sex-all-sex, fig.height = 3.5, fig.cap= "Relative sex counts (users from all age and on any birth control type, any sex log) and trend in relative sex counts"}
...
```{r birth-models-comparing-additive-vs-multiplicative-decompositions, fig.width=10, fig.height=5, out.width='100%', fig.cap="Comparison of mutliplicative (accounts for variation of the variance with the number of samples) vs additive (does not account) detrending."}
...
```{r birth_models multiplicative decomposition}
...
```{r birth-models-multiplicative-decomposition-VIZ, fig.height=4, fig.cap="Detrended relative changes in sexual behavior (all users of all age group on any birth control type, all sex)."}
...
ok = foreach(ca = unique(clue_sex_agg$country_area)) %do% {
  ok = foreach(bc = unique(clue_sex_agg$BC)) %do% {
    ok = foreach(ac = unique(clue_sex_agg$age_cat)) %do% {
...
```{r birth_models save workspace, cache = FALSE, eval = FALSE}
